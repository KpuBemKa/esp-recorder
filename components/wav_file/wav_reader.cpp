...
WavReader::Open(const std::string_view file_path)
{
  ...
  if (m_wav_header.bit_depth != 16) {
    ESP_LOGE(
      TAG, "ERROR: bit depth %d is not supported\n", m_wav_header.bit_depth);
  }

  if (m_wav_header.num_channels != 1) {
    ESP_LOGE(
      TAG, "ERROR: channels %d is not supported\n", m_wav_header.num_channels);
  }

  ESP_LOGI(TAG,
           "fmt_chunk_size=%d, audio_format=%d, num_channels=%d, "
           "sample_rate=%d, sample_alignment=%d, bit_depth=%d, data_bytes=%d\n",
