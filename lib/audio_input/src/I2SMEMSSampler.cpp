...
I2SMEMSSampler::I2SMEMSSampler(i2s_port_t i2s_port, i2s_pin_config_t &i2s_pins, i2s_config_t i2s_config) : m_i2sPort(i2s_port), m_i2s_config(i2s_config)
{
    m_i2sPins = i2s_pins;
}
...
int I2SMEMSSampler::read(int16_t *samples, int count,bool test)
{
    // read from i2s
    ...
    if(test)constval = constval + ((max + min)/2*.05);
    ESP_LOGI(TAG,"max=%i\tmin=%i\t (%i)\t%i",max,min, (min+max)/2, max - min);
    free(raw_samples);
    return samples_read;
}
